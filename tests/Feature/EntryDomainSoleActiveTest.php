...
it('轮换备用域名后，它成为唯一的在用域名', function () {
    ...
    $this->actingAs(esaAdmin())->post("/admin/entry-domains/{$b->id}/rotate", ['pointed_ip' => '5.6.7.8']);
...
it('设为在用仍然只留一个', function () {
    ...
    $this->actingAs(esaAdmin())->post("/admin/entry-domains/{$b->id}/activate");
...
it('删掉最后一个在用域名时，提示说清会回退发裸 IP', function () {
    ...
    $this->actingAs(esaAdmin())->delete("/admin/entry-domains/{$a->id}");
...
it('还有别的在用域名时不误报', function () {
    ...
    $this->actingAs(esaAdmin())->delete("/admin/entry-domains/{$b->id}");
