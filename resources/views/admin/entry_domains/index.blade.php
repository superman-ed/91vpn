  ...
  <div class="card-body">
    ...
    <div class="table-responsive">
      <table class="table table-striped">
        <thead><tr>
          <th>入口域名</th><th>所属节点</th><th>状态</th><th>指向 / DNS</th><th>上次轮换</th><th>操作</th>
        </tr></thead>
        <tbody>
        @forelse($domains as $d)
          <tr>
            ...
            <td>
              ...
              @if($d->dnsStale())
                <br><span class="adm-pill" style="background:#fff3cd;color:#664d03" title="A 记录指的 IP 和节点当前真实 IP 对不上">
            ...
            <td style="min-width:280px">
              ...
              <form method="POST" action="/admin/entry-domains/{{ $d->id }}" class="d-inline"
                    data-dgr="删除后订阅将回退发该节点的裸 IP。">@csrf @method('DELETE')
                <button class="btn btn-outline-danger btn-sm">删除</button></form>
            </td>
          </tr>
        @empty
          <tr><td colspan="6"><div class="adm-empty">还没有入口域名。给节点挂一个稳定域名，客户端就连域名而非裸 IP。</div></td></tr>
        @endforelse
        </tbody>
      </table>
    </div>
