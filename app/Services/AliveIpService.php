...
class AliveIpService
{
    ...
    /**
     * `[!!]` 【2026-09-23 起没有调用方】—— 额度改按设备算（`DeviceService`），
     * 不再按 IP 踢人。`UserController::aliveIp` 现在一律返回空的 blocked 列表。
     *
     * 保留本方法而不删：决定刚做、产品尚未上线，可能回滚；它的逻辑本身是对的，
     * 下面那组测试也仍在守着「踢谁」的判据。
     * **但要接回去之前，先读 `docs/decisions/device-model.md` 与清单 L-08** ——
     * 按 IP 踢人两个方向都错，改排序只能减轻、改不了根子。
     *
     * ----
     *
