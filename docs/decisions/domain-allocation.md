...
### `[!!]` 2026-09-24 补修:节点回连地址一度跟着 APP_URL 跑到了牺牲域

`[D]` `RelayDeployController::identity()` 原本读 `config('app.url')` 作为
节点的 `api_url`。`APP_URL` 改成 `91vpn.com` 之后，**新部署的节点会被配置
去回连投广告的那个域** —— 与本文的分配正好相反。已有的 #100 用的还是老地址，
所以没坏，但每台新机器都会带上错的地址。

现已改读独立的 `NODE_API_URL`（`config/app.php` 的 `node_api_url`，
不配则回落 `APP_URL`），生产配为 `https://app.91app.shop`。
这样官网域名以后再怎么换，**节点这条回连专线不跟着动**。

`[!]` 封了之后的表现很难发现：节点不会停，它继续按上次拿到的用户名单服务
（fail-open，见 L-11）—— 新买的用户连不上、已过期的还能用、流量完全不计，
而且没有任何告警。面板挂了会立刻知道；节点回连不上，可能好几天都不知道。
