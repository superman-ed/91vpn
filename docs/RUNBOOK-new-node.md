...
`install.sh` 会把当前值打进摘要：

```
==> 拥塞控制 cubic / qdisc fq_codel（未改动；要改用 --congestion=bbr|cubic）
```

`[!!]` **默认不改是刻意的。** BBR 并非在所有链路上都优于 CUBIC —— BBRv1 在浅缓冲 /
共享瓶颈上会过量发送、对 CUBIC 不公平；哪个更好取决于实际 RTT、丢包位置与是否随机、
并发数、上下行是否对称，**只能 A/B 实测**。脚本的职责是让它可见、可复现、可按机器切换，
不是替你下结论。

要切换（会持久化到 `/etc/sysctl.d/99-agent-net.conf`）：

```bash
bash install.sh --congestion=bbr    # 或 cubic
```

`[!]` 脚本一律从 `/proc/sys` 读，不依赖 `sysctl` 命令 ——
`[D]` 实测精简镜像上 `sysctl` 可能不存在，而 `/proc/sys` 照样可读。
内核不提供该算法时**拒绝而不是静默保持旧值**（否则摘要会撒谎）。
