...
it('credits rebate to inviter when downline pays', function () {
    $inviter = User::factory()->create(['money' => 0]);
    $downline = User::factory()->create(['ref_by' => $inviter->id, 'class' => 0, 'class_expire' => now()->subDay(), 'transfer_enable' => 0]);
    ...
    // 默认返利 20% → 50 * 0.2 = 10
    expect((float) $inviter->fresh()->money)->toBe(10.0);
    expect(Payback::where('user_id', $inviter->id)->first()->amount)->toEqual('10.00');
});
