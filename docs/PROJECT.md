...
> 正文（§1 起）最后成段更新于 2026-09-04；**§0 于 2026-09-23 重写**。

`[!!]` **§1–§7 有 19 天的漂移**：其间累计 179 个提交，
入口域名、REALITY、设备模型、请求头鉴权、中转合并（ADR-008）都是之后做的。
读那几节时以**下面 §0 和这三份文档为准**：

| 要知道什么 | 看哪份 |
|---|---|
| 怎么在新机器上搭起来 | **`docs/DEPLOYMENT.md`**（2026-09-23 新增） |
| 上线前还欠什么 | `docs/LAUNCH-CHECKLIST.md`（持续更新） |
| 某个设计为什么这么定 | `docs/decisions/`（D-1 … D-6） |

---

## 0. 一句话现状（2026-09-23）

**一条完整链路已经真机跑通**：用户 → 订阅（发入口域名，不发裸 IP）→ 香港落地节点
（VLESS + REALITY + vision）→ 出网。手机客户端实测连上并跑过流量。

```
面板      Laravel + docker compose(app/scheduler/web/cloudflared/db/redis)
          后台双层隔离:ADMIN_HOST 中间件 + nginx 回环端口
节点      自研 Go agent,mod_mu/sspanel 兼容;密钥走 X-Node-Secret 头
客户端     Android / Windows 自研已可用;iOS / macOS 未做,那两端走第三方客户端
测试      面板 970 passed;agent 37 个包全过
```

**离"能收钱"只差一件事**：`[!!]` **epay 真实对接** —— 三项配置都有值、
`configured()` 返回 true，但**从未有过一笔带网关交易号的支付**。
字段非空 ≠ 对接完成（清单 L-02）。

**已明确决定不做的**（免得后人重新纠结）：

- `[decided]` **按设备发代理凭据**（D-6 阶段二）—— 收入模型是流量配额，
  而套餐从未承诺过设备数。设备限制只对自研客户端生效，这是接受的取舍。
- `[decided]` **按 IP 踢人** —— 两个方向都错（共用出口少算、换网多算），已停用。

**知道但还没处理的**（都在清单里，不阻塞上线）：
dest 挂掉无人告警（L-22）、fail-open 无时间上界（L-11，真机撞到过 15 天）、
`traffic_logs` 是死表（L-18）。
