...
@forelse($plans as $p)
<tr><td>{{ $p->id }}</td><td>{{ $p->name }}</td><td><span class="badge badge-light">{{ period_name($p->period) }}</span></td><td>¥{{ $p->price }}</td><td>{{ $p->transfer_gb }}GB</td><td>{{ $p->class }}</td><td>{{ $p->speed_limit }}M/{{ $p->ip_limit }}台</td><td>{{ $p->duration_days }}天</td>
