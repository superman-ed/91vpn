...
<thead><tr><th>ID</th><th>名称</th><th>价格</th><th>流量</th><th>等级</th><th>限速/设备</th><th>时长</th><th>在售</th><th>操作</th></tr></thead><tbody>
@forelse($plans as $p)
<tr><td>{{ $p->id }}</td><td>{{ $p->name }}</td><td>¥{{ $p->price }}</td><td>{{ $p->transfer_gb }}GB</td><td>{{ $p->class }}</td><td>{{ $p->speed_limit }}M/{{ $p->ip_limit }}台</td><td>{{ $p->duration_days }}天</td>
...
@empty<tr><td colspan="9" class="text-muted">暂无套餐</td></tr>@endforelse
