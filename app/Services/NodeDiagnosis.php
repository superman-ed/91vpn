...
class NodeDiagnosis
{
     ...
     * dest 挂掉时用它的落地全部同时失效(新连接全断,包括密钥正确的老用户)。
     * 这一条不需要任何对手模型就成立。
     * `[!]` "多台落地共用会更易被关联识别"是 [I] 推测,未实证 ——
     * 措辞里不把它写成结论。
     */
    private function sharedDest(Node $node): ?array
    {
        ...
        return $this->x('warn', 'dest 共用',
            ...
            .'主要问题是**故障爆炸半径**：这个 dest 挂掉时，这 '
            .($others->count() + 1).' 台会【同时】新连接全断'
            .'（REALITY 在读 ClientHello 之前就要连上 dest，连不上直接断，'
            .'密钥正确的老用户也一样）。我们踩过一次：mirrors.xtom.com 当天挂掉。'
            .'另外每条新连接都要连一次 dest，共用时它承受的是几台之和。'
            .'建议按地区分组，把故障域拆开');
    }
