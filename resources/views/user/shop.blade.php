...
@section('content')
@php
    $periodTabs = ['month' => '1月', 'quarter' => '3月', 'half_year' => '6月', 'year' => '12月'];
    $grouped = $plans->groupBy('period');
    $activeTabs = collect($periodTabs)->filter(fn ($label, $key) => $grouped->has($key));
@endphp
...
<ul class="nav nav-pills mb-4">
    ...
    <li class="nav-item"><a class="nav-link {{ $loop->first ? 'active' : '' }}" data-toggle="pill" href="#tab-{{ $key }}">{{ $label }}</a></li>
    @endforeach
</ul>
<div class="tab-content">
    ...
    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="tab-{{ $key }}">
        <div class="row shop-row">
            @foreach($grouped[$key] as $plan)
            @php $soldOut = $plan->stock === 0; @endphp
            <div class="col-12 col-md-6 col-lg-4">
                <div class="card plan-card">
                    <div class="card-header">
                        <h4 style="color:#6777ef">{{ $plan->name }}</h4>
                        <div class="card-header-action"><span class="badge badge-light">{{ $label }}</span></div>
                    </div>
                    <div class="card-body">
